...
class CategoryDocRu(CategoryDoc):
...
class CategoryDocExtraLang(CategoryDoc):
    language = models.ForeignKey('main.ExtraLanguage', on_delete=models.CASCADE, verbose_name='Язык')
    category_rus = models.ForeignKey(CategoryDocRu, on_delete=models.CASCADE, verbose_name='Категория', help_text='Выберите категорию, которую хотите перевести')
...
class Document(models.Model):
    category = models.ForeignKey(CategoryDocRu, on_delete=models.CASCADE, verbose_name='Категория')
    document = models.FileField('Документ', upload_to='local_lore/%Y/%m/', max_length=100)
    description = models.TextField('Описание', blank=True, null=True)
    datetime = models.DateTimeField('Дата', auto_now=False, auto_now_add=True)

    def __str__(self):
        return f'{self.category} - {os.path.basename(self.document.name)}'
...
class DescriptionDocExtraLang(models.Model):
    document_rus = models.ForeignKey(Document, on_delete=models.CASCADE, verbose_name='Документ')
    language = models.ForeignKey('main.ExtraLanguage', on_delete=models.CASCADE, verbose_name='Язык')
